...
## 2. 当前精确状态(E8 已推送,E9 已复审通过待提交)

- HEAD:5e0f354 E8 "literature: close claim-specific citation support"。
  E7 为 19e19ff,E6 为 eeea1cf。
...
- E6:Stage 18 citation-count detector 扩展为版本化闭集
  ...
  已提交并推送为 eeea1cf。
- E7 已提交并推送,只读复审 APPROVE,无 P0/P1:Stage 23 固定读取 canonical
  ...
  bounded-bib closure、无 canonical bib、symlink paper 对抗测试。
- E8 已提交并推送,只读复审 APPROVE,无 P0/P1:新增 strict
  `citation_support.json`;每个最终 citation
  occurrence 绑定 exact sentence、final plan claim、retained excerpt 的 Stage 4
  path/hash/span 与 Stage 23 existence status。critic 显式使用
  `paper_revision.critic_model`,与 writer 同名直接 FAILED,且只能输出
  supported/unsupported+reason。仅 verified existence 可 supported;无 critic、
  malformed response、任一 unsupported 或 dataset-origin contradiction 在所有
  scope FAILED。`citations.json` 由 closure 派生,unsupported 保持 unmapped,
  禁 background 逃逸。Stage 24 固定读取 Stage 23 正文并与 Stage 22 byte
  identity 对账,入口清 stale outputs。复审后补 writer==critic、existence
  覆盖、Stage22/23 正文分叉、verification key closure 对抗测试,并在 spec
  明确非 citation unsupported claim 的诊断边界。
- E9 working tree 已获只读复审 APPROVE,无 P0/P1:新增
  `citation_release_audit.py` 单一磁盘重放器,
  从 active run config + canonical Stage 9 contract 开始,依次重放 Stage 4
  registry、Stage 5 deterministic prefilter/rank/top-150 admission、cards、
  allowlist、effective policy、final plan、Stage 17 experiment/citation closure、
  Stage 22/23 final-paper + verification result/count/completeness closure 与
  Stage 24 support/claims/citations/truth
  bindings。release_check 只转 typed audit error 为 finding;citation 错误不在
  degraded-compatible closed set。复审后补真实 ReleaseChecker→E9 接线、
  allow_suspicious 不可豁免、Stage24 cross-artifact 分项篡改测试;spec 明确
  E9 是唯一 release 级 citation authority,旧 checks 仅兼容诊断;并细分
  Stage6-17 replay 错误码/路径。尚未 commit。
...
- d4db0ab CITATION_EVIDENCE_PIPELINE_SPEC(v1 abstract-only + v2 HITL 全文,
  ...
  e9a7fbd E2;42b3dc1 E3;9f05e69 E4/E5;eeea1cf E6;19e19ff E7;
  5e0f354 E8;E9 WIP(见 §2)。
...
- E6 已完成:Stage 18/20 policy 一致性(窄数字检测,一次 review 重生成,禁
  not_actionable 逃逸)。
- E7 已完成:Stage 23 有界验证(只验实引集合;删除
  `_remove_citations_from_text` 的静默改稿 authority;严格 scope 完整性门禁)。
- E8 已完成:Stage 24 claim-specific support + dataset-origin truth audit。
- E9 已复审通过待提交:release replay(gate 类,独立 commit)。
