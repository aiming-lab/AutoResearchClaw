...
## 2. 当前精确状态(E3 复审通过,待 commit)

- HEAD:e9a7fbd "literature: seal Stage 6 evidence cards"(= E2 已提交)。
...
- E3:**working tree 未提交,已通过全量复审 + 定点确认(APPROVE,可
  commit)**。内容:顶层 citation_policy 严格双层解析(_strict_int,
...
- 下一步:commit E3 → E4(Stage 16 citation plan)+ E5(Stage 17 封闭
  写作面),**E4/E5 必须同发布窗口**,中间 checkout 不得启动 pipeline。
