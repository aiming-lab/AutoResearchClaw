...
class TestAcpSessionEnsure:
    def test_ensure_session_uses_existing_listed_session(self):
        from researchclaw.llm.acp_client import ACPClient, ACPConfig

        client = ACPClient(ACPConfig(agent="codex", session_name="researchclaw"))
        client._acpx = "/usr/bin/true"

        with mock.patch.object(client, "_session_exists", return_value=True) as exists_mock:
            client._ensure_session()

        assert client._session_ready is True
        exists_mock.assert_called_once_with("/usr/bin/true")

    def test_ensure_session_waits_for_background_owner(self, monkeypatch: pytest.MonkeyPatch):
        from researchclaw.llm.acp_client import ACPClient, ACPConfig
        ...
        session_checks = iter([False, False, True])
        monkeypatch.setattr(client, "_session_exists", lambda acpx: next(session_checks))
        monkeypatch.setattr("researchclaw.llm.acp_client.subprocess.Popen", lambda *args, **kwargs: _DummyProc())
        monkeypatch.setattr("researchclaw.llm.acp_client.time.sleep", lambda _: None)

        client._ensure_session()

        assert client._session_ready is True

    def test_session_exists_ignores_closed_sessions(self, monkeypatch: pytest.MonkeyPatch):
        from researchclaw.llm.acp_client import ACPClient, ACPConfig

        client = ACPClient(ACPConfig(agent="codex", session_name="researchclaw", cwd="/tmp/project"))
        client._acpx = "/usr/bin/true"

        completed = mock.Mock(returncode=0, stdout=(
            "019d [closed]\tresearchclaw\t/tmp/project\t2026-03-18T20:00:00.000Z\n"
            "0200\tother\t/tmp/project\t2026-03-18T20:00:01.000Z\n"
        ))
        monkeypatch.setattr(
            "researchclaw.llm.acp_client.subprocess.run",
            lambda *args, **kwargs: completed,
        )

        assert client._session_exists("/usr/bin/true") is False
